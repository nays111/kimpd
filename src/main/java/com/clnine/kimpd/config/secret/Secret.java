package com.clnine.kimpd.config.secret;

public class Secret {
    public static String JWT_SECRET_KEY = "UwKYibQQgkW7g-*k.ap9kje-wxBHb9wdXoBT4vnt4P3sJWt-Nu";
    public static String USER_INFO_PASSWORD_KEY = "o9pqYVC9-F8_.PEzEiw!L9F6.AYj9jcfVJ*_i.ifXYnyE68kix@Q2dL6rw*bV-rpdZYwcqZG-jPF-fw3CiJyKsfZ778ks-*jnZn";
    public static String aligoKey="bt1qufu4xrtcwpi3bsp6i7tyiue7cufy";
    public static String aligoId="kimpdlabs";
    public static String aligoSender="01062376451";
    public static String barobillCertyKey = "1CD1F6A5-33A6-40E0-B5D6-0A4256685139";
    public static String barbobillCorpNum = "2648100466";
}
